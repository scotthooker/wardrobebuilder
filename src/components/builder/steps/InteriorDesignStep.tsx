/**
 * InteriorDesignStep Component
 * Summary view of interior configurations completed in CarcassLayoutStep
 */

import { Card } from '@/components/ui/Card'

// Type definitions for interior sections
type InteriorSectionType = 'drawers' | 'rail' | 'shelves' | 'double_rail' | 'empty'

interface InteriorSection {
  id: number
  type: InteriorSectionType
  height: number
  drawers?: number
  shelfCount?: number
  isExternal?: boolean
...
interface InteriorDesignStepProps {
...
export function InteriorDesignStep({ configuration }: InteriorDesignStepProps) {
  ...
  const drawerSections = configuration.sections.reduce((sum, s) =>
    sum + s.carcasses.reduce((cSum, c) =>
      cSum + (c.interiorSections?.filter(i => i.type === 'drawers').length || 0), 0
    ), 0
  )

  const shelfSections = configuration.sections.reduce((sum, s) =>
    sum + s.carcasses.reduce((cSum, c) =>
      cSum + (c.interiorSections?.filter(i => i.type === 'shelves').length || 0), 0
    ), 0
  )

  const railSections = configuration.sections.reduce((sum, s) =>
    sum + s.carcasses.reduce((cSum, c) =>
      cSum + (c.interiorSections?.filter(i => i.type === 'rail' || i.type === 'double_rail').length || 0), 0
    ), 0
  )

  const emptyCarcasses = configuration.sections.reduce((sum, s) =>
