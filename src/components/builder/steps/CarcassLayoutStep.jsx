...
export function CarcassLayoutStep({ configuration, updateConfiguration }) {
  // Materials hook
  const { materials, isLoading: materialsLoading, getDefaultMaterialOption } = useMaterials();

  // Wizard state
  const [wizardStep, setWizardStep] = useState('SELECT_COLUMNS');
  ...
  const [currentCarcassMaterial, setCurrentCarcassMaterial] = useState(null);
  ...
  const handleCarcassComplete = (interiorSections, addAnother = false) => {
  ...
  const handleEdit = (columnIdx, carcassIdx) => {
...
function SelectColumnsView({ numberOfColumns, setNumberOfColumns, onNext, wardrobeWidth }) {
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg p-8">
      ...
      <button
        onClick={onNext}
        className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition font-semibold text-lg"
      >
...
function SetCarcassHeightView({
  ...
  const handleMaterialSelect = (materialName, thicknessOption) => {
    onMaterialChange({
  ...
  return (
    <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-lg p-8">
              ...
              <div key={material.name} className="border-2 border-gray-200 rounded-lg p-4 hover:border-primary-500 transition">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {material.options.map((option) => (
                    <button
                      key={option.sku}
                      onClick={() => handleMaterialSelect(material.name, option)}
                      className={`p-3 border-2 rounded-lg text-sm transition ${
                        selectedMaterial?.sku === option.sku
                          ? 'border-primary-600 bg-primary-50 text-primary-900 font-semibold'
                          : 'border-gray-300 hover:border-primary-400'
                      }`}
                    >
                      <div className="font-semibold">{option.thickness}</div>
                      <div className="text-xs text-gray-600 mt-1">£{option.price}</div>
                    </button>
                  ))}
                </div>
              </div>
      ...
      <div className="flex gap-4">
        <button
          onClick={onBack}
          className="flex-1 flex items-center justify-center gap-2 px-6 py-4 border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition font-semibold text-lg"
        >
        ...
        <button
          onClick={onNext}
          className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition font-semibold text-lg"
        >
...
function ConfigureCarcassView({
  ...
  const [interiorSections, setInteriorSections] = useState(currentInterior.length > 0 ? currentInterior : []);

  const addInteriorSection = (type) => {
    ...
    const newSection = {
      ...
      isExternal: type === 'drawers' ? false : null // Drawers can be internal or external
    };

    setInteriorSections([...interiorSections, newSection]);
  };

  const removeInteriorSection = (index) => {
    setInteriorSections(interiorSections.filter((_, i) => i !== index));
  };

  const updateInteriorHeight = (index, height) => {
    const updated = [...interiorSections];
    updated[index] = { ...updated[index], height: parseInt(height) || 0 };
    setInteriorSections(updated);
  };

  const updateInteriorCount = (index, field, count) => {
    const updated = [...interiorSections];
    updated[index] = { ...updated[index], [field]: parseInt(count) || 1 };
    setInteriorSections(updated);
  };
    ...
    <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-lg p-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.values(SECTION_TYPES).map((type) => (
                <button
                  ...
                  className="p-4 border-2 border-gray-300 rounded-lg hover:border-primary-500 hover:bg-primary-50 transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
      ...
      <div className="space-y-4">
        <div className="flex gap-4">
          <button
            onClick={onBack}
            className="flex items-center justify-center gap-2 px-6 py-3 border-2 border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition font-medium"
          >
        ...
        {canAddAnother && (
          <button
            ...
            className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-semibold text-lg"
          >
...
function ReviewView({ sections, wardrobeWidth, wardrobeHeight, onEdit, onBack }) {
  return (
    <div className="bg-gradient-to-br from-green-50 to-blue-50 rounded-lg p-8">
