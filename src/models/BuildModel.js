...
export class BuildModel {
  constructor(buildConfig, baseConfig, pricingData) {
    // Basic properties
  ...
  _calculateCosts() {
    // 1. Material costs
    const materialCosts = Object.entries(this.materials).map(([key, mat]) => {
    ...
    // 4. Extras costs
    const extrasCosts = Object.entries(this.extras).map(([key, extra]) => ({
      item: key,
      desc: extra.desc,
   ...
   */
  _findPricing(materialName, thickness) {
   ...
   */
  updateMaterial(key, updates) {
    this.materials[key] = { ...this.materials[key], ...updates };
    this.costs = this._calculateCosts();
    return this;
  }
   ...
   */
  updateExtra(key, updates) {
    this.extras[key] = { ...this.extras[key], ...updates };
    this.costs = this._calculateCosts();
    return this;
  }
